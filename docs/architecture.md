...
5. ~~**Pre-commit hook installation path.**~~ **Resolved 2026-05-16 (step 7):** chose file-copy into `.git/hooks/pre-commit` for visibility. Marker comment in the script identifies it as ours; install backs up any existing foreign hook to a timestamped sibling; uninstall refuses to remove non-harness hooks. Implementation: `src/pii/precommit.ts`. CLI: `harness pii-check --install-hook` / `--uninstall-hook`.
...
5. ✓ **Done (text-only minimum-viable, uncommitted).** Telegram bot (grammY) with owner-chat-id gating, conversation buffer (§17.6 #3), wedge watchdog (§17.4 #1), polling-rejection handler (§17.4 #2), 4096-chunking + Markdown fallback (§17.4 #4/#5), typing keep-alive (§17.4 #7), slash commands (§17.4 #8), uncaught-exception handling (§17.7 #1/#2/#3). End-to-end chat works against MemPalace bridge (currently UNIMPLEMENTED ops) + Claude CLI wrapper. Files: `src/bot/{messaging,access,conversation-buffer,error-log,watchdog,index}.ts`, `src/bot/handlers/text.ts`, `src/cli/commands/bot.ts`.
   - 5b. ✓ **Done (uncommitted).** Multi-modal + streaming + compaction. Voice handler (downloads .ogg from Telegram, transcribes via opt-in `scripts/transcribe.sh` faster-whisper venv, echoes transcript with mic prefix, routes through text pipeline). Photo handler (downloads largest variant, instructs Claude to use Read tool on the file path). Document handler (image-MIME → photo flow; non-image → polite acknowledgment). Progressive Telegram editing during stream (placeholder + editMessageText every 3s + final edit OR delete+chunk if >4096). Proactive token compaction at config.claude.token_compaction_percent (default 60% of 1M = 600K tokens — archive buffer to MemPalace, clear, start fresh). All three multi-modal handlers route through new `processTextTurn` export of text handler so they share the prompt assembly + claude invocation + buffer + journal logic. Files: `src/bot/file-download.ts`, `src/bot/handlers/{voice,photo,document,text}.ts`, `scripts/{install-transcribe,transcribe}.sh`.
6. ✓ **Done (uncommitted).** Cron runner with §17.3 patterns: per-job model/timeout/persistent_session, anti-self-talk directive, same-minute dedup via minuteKey, tick-stall watchdog, post-restart catch-up after 60s settling, skip-catchup one-shot marker, self-healing Telegram alert on max-attempts failure, catch-up triple-check (in-memory → journal; gmail-check deferred to step 9). 2 example jobs ship in `examples/cron/`. Files: `src/cron/{types,loader,schedule,journal,telegram,runner,catchup,scheduler}.ts`, `src/cli/commands/cron.ts`, `examples/cron/{morning-briefing,weekly-reflection}.md`.
7. ✓ **Done (uncommitted).** PII tooling per §18.1: `harness pii-check` (full personal/ scan), `harness pii-check --staged` (out-of-personal/ check for the hook), `harness pii-check --install-hook` / `--uninstall-hook` (opt-in `.git/hooks/pre-commit`), `harness memory purge --all` (two-step confirm + bridge-then-filesystem fallback) / `--query` / `--range` (defer with friendly error until MemPalace exposes delete ops), `harness memory stats` (bridge + filesystem). Scanner reports category counts + line numbers only — never the matched values themselves. Files: `src/pii/{patterns,scanner,precommit}.ts`, `src/cli/commands/{pii-check,memory}.ts`.
8. ✓ **Done (uncommitted; MVP).** `harness setup` interactive wizard per §7: privacy gate, owner identity (name, timezone, assistant name), Telegram token + chat_id (with --chat-id escape hatch), Google opt-in flag (adapter lands step 9), template rendering via Mustache with HTML-escape disabled, cron-defaults seed when personal/cron/ is empty, PII opt-ins (precommit hook + restricted mode), final doctor re-run. Idempotent: re-running pre-fills from existing personal/config.yaml and skips written files unless --force (which creates .bak sidecars). Templates ship in `templates/` (CLAUDE.md, identity.md, principles.md, wiki/index.md, wiki/follow-ups.md, wiki/open-questions.md + the verbatim Karpathy WIKI.md). Files: `src/setup/{prompts,templates,wizard}.ts`, `src/cli/commands/setup.ts`. **Deferred to step 8b:** live chat_id auto-capture by listening for one bot message (currently user provides via flag/prompt).
9. ✓ **Done (uncommitted; MVP).** Google adapter (Gmail + Calendar) via `googleapis` + `google-auth-library`. OAuth flow runs a temp HTTP server on a free port to catch the callback (Google deprecated the paste-code/oob flow); refresh token persisted to `.env` with chmod 600, preserving other lines. Gmail: searchMessages, sendMessage, buildRfc822Message, wasSubjectSentRecently (powers the §17.3 #9 (c) catch-up gmail-check). Calendar: listUpcoming, createEvent. Cron scheduler wires gmail-check automatically when `config.google.enabled` AND `GOOGLE_*` env vars are present; gracefully degrades to in-memory+journal-only otherwise. Cron job frontmatter gains optional `gmail_subject` field for the check to use. CLI: `harness google login` (OAuth) + `harness google test` (connectivity smoke). Files: `src/adapters/google/{oauth,client,gmail,calendar,index}.ts`, `src/cli/commands/google.ts`. **Deferred:** Drive ops and advanced Calendar (recurring events, multi-calendar listing) — step 9b. Live OAuth + Gmail/Calendar live API tests require user-side Google Cloud project; CI covers offline paths only.
10. ✓ **Done (uncommitted).** `harness start` single-process orchestrator per §9(a) (MemPalace bridge + bot + cron scheduler with shared shutdown), `ecosystem.config.cjs` for pm2 with the §17.4/§17.5 resilience settings (kill_timeout 10s for bridge SIGTERM grace, max_memory_restart 1G), `examples/systemd/claude-pmax-harness.service` power-user alternative with install + auto-boot guidance. Files: `src/cli/commands/start.ts`, `ecosystem.config.cjs`, `examples/systemd/claude-pmax-harness.service`.
11. ✓ **Done (uncommitted).** `.github/workflows/ci.yml` (typecheck + test matrix on ubuntu+macos, Node 20+22, with Python 3.11 setup; plus a PII guard job that runs the scanner against src/test/examples/templates/scripts/docs and fails on findings). `PRIVACY.md` polished from the PLAN.md draft. `README.md` rewritten from 3-line stub to a real first-impression doc with prereqs / quickstart pointer / CLI surface / privacy summary. `docs/quickstart.md` end-to-end install + setup walkthrough. `LICENSE` (MIT) added.
